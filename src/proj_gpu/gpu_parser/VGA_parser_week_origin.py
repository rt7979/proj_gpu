...
def weekly_dataframe(records: list[dict[str, str]]) -> pd.DataFrame:
    """Group price records by graphics-card model and calendar week."""
    columns = ["顯卡型號", "日期(以週為單位)", "最低價格", "最高價格", "平均價格"]
    if not records:
        return pd.DataFrame(columns=columns)

    frame = pd.DataFrame(records).rename(columns={"product": "顯卡型號"})
    frame["日期"] = pd.to_datetime(frame["date"], errors="coerce")
    frame["價格"] = pd.to_numeric(frame["price"], errors="coerce")
    frame = frame.dropna(subset=["顯卡型號", "日期", "價格"])
    frame["日期(以週為單位)"] = frame["日期"].dt.to_period("W-SUN").dt.start_time
    category = frame.get("category", pd.Series("", index=frame.index)).str.lower()
    known_series = category.str.endswith(("_min", "_max", "_avg"))
    frame["最低"] = frame["價格"].where(~known_series | category.str.endswith("_min"))
    frame["最高"] = frame["價格"].where(~known_series | category.str.endswith("_max"))
    frame["平均"] = frame["價格"].where(~known_series | category.str.endswith("_avg"))

    weekly = (
        frame.groupby(["顯卡型號", "日期(以週為單位)"], as_index=False)
        .agg(最低價格=("最低", "min"), 最高價格=("最高", "max"), 平均價格=("平均", "mean"))
    )
    return weekly[columns].sort_values(
        ["顯卡型號", "日期(以週為單位)"]
    ).reset_index(drop=True)


def save_csv(dataframe: pd.DataFrame, output: Path) -> None:
    output.parent.mkdir(parents=True, exist_ok=True)
    dataframe.to_csv(output, index=False, encoding="utf-8-sig", date_format="%Y-%m-%d")
...
def main() -> None:
    ...
    weekly = weekly_dataframe(records)
    save_csv(weekly, args.output)

    if not weekly.empty:
        print(f"Saved {len(weekly)} weekly records to {args.output}")
        print(
            f"Available range: {weekly.iloc[0]['日期(以週為單位)'].date()} "
            f"to {weekly.iloc[-1]['日期(以週為單位)'].date()}"
        )
    else:
